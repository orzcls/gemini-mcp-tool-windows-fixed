...
async function executeCommandWithPipedInput(command, input, env, onProgress, customPowershellPath = null) {
    return new Promise((resolve, reject) => {
        ...
        const completeEnv = {
            ...
            PATH: process.env.PATH || '',  // Ensure PATH is always available
...
`;
    ...
    if (selectedModel !== MODELS.PRO) {
        args.push(CLI.FLAGS.MODEL, selectedModel);
    } else {
        // Explicitly set the default model to ensure 2.5 series is used
        args.push(CLI.FLAGS.MODEL, MODELS.PRO);
    }
