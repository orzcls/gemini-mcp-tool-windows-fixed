...
async function executeCommandWithPipedInput(command, input, env, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        const childProcess = spawn('powershell.exe', ['-Command', `${command} ${args.join(' ')}`], {
            env: env,
            stdio: ['pipe', 'pipe', 'pipe']
        });
        
        console.log(`[GMCPT] Spawned process with command: powershell.exe -Command "${command} ${args.join(' ')}"`);
...
async function executeCommandWithEnv(command, args, env, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        const childProcess = spawn('powershell.exe', ['-Command', psCommand], {
            env: env,
            stdio: ['pipe', 'pipe', 'pipe']
        });
