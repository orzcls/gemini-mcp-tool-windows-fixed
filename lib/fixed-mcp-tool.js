    ...
    try {
        switch (name) {
            case "ask-gemini":
                ...
                const result = await executeGeminiCLI(prompt, model, sandbox, changeMode);
            ...
            case "brainstorm":
                ...
                console.error('[GMCPT] brainstorm tool called with prompt: ' + brainstormPrompt.substring(0, 50) + '...');
                ...
                const brainstormResult = await executeGeminiCLI(enhancedPrompt, brainstormModel, false, false);
                return {
                    content: [{
                        type: "text",
